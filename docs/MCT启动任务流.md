...
└─ Program.Main(args)
    ...
    │   └─ --lang <语言> → overrideLang（覆盖配置语言）
    │
    ├─ I18nManager.Instance.InitializeDefault() — 国际化默认初始化（见附录H-1）
    │   └─ 加载内嵌 en 翻译，确保后续日志不输出翻译键
    │
    ├─ 设置日志级别
    │   ├─ isDebugMode? → Console=Debug, File=Debug
    │   └─ 非调试 → Console=Info, File=Debug
    │
    ├─ isTestMode? → 禁用插件签名验证（VerifyDll 直接返回 true）
    │
    ├─ 打印 ANSI 启动画面
    │
    ├─ new MorningCatBot(exitCallback, isTestMode, isDebugMode, overrideLang)
    │   ├─ _i18n.InitializeDefault() — 国际化默认初始化（同附录H-1）
    │   │   └─ 确保构造函数内所有组件可使用翻译
    │   ├─ new MessageDistributionCore() + OneBotPlatformAdapter
    │   ├─ new ConfigManager()
    │   │   ├─ config.yml 存在?
    │   │   │   ├─ 是 → 反序列化加载
    │   │   │   │   ├─ 配置完整 → 直接使用
    ...
    │   ├─ new PluginSignatureVerifier(configManager, testMode) — 见附录G
...
├─ _isRunning? → 是 → 直接返回
│
├─ _i18n.Initialize(lang, baseDir) — 国际化完整初始化（见附录H-2）
│   ├─ 确定语言: overrideLang ?? config.Lang
│   ├─ 创建 lang 目录
│   ├─ 解压内嵌 zh.yml 和 en.yml
│   ├─ 扫描可用语言
│   ├─ 加载目标语言文件
│   ├─ 检查内嵌语言包完整性（缺失键则覆盖修复）
│   ├─ 初始化失败 → Log.Error + throw Exception
│   └─ 初始化成功 → Log.Info "i18n.initialized"
│
├─ await FetchPublicKeyAsync() — 拉取插件签名公钥（见附录G-1）
│   ├─ 从服务器获取公钥
│   │   ├─ 获取成功?
│   │   │   ├─ 是 → 公钥与本地不同? → 更新并保存到配置
│   │   │   └─ 否 → 跳过
│   │   └─ 获取失败
│   │       ├─ 本地有缓存公钥 → 使用本地缓存
│   │       └─ 本地无缓存公钥 → _publicKey 为空，后续 VerifyDll 跳过验证
│   └─
│
...
│   │
│   ├─ SubscribeModuleManagerEvents() — 注册模块事件（含签名验证）
│   │   ├─ OnBeforeModuleLoad → 签名验证（见附录G-2）
│   │   │   ├─ 验证通过 → ModuleLoadAction.Continue
│   │   │   └─ 验证失败 → 记录到 _signatureFailedModules，返回 ModuleLoadAction.Skip
│   │   ├─ OnProgressUpdated → 日志进度
...
└─ Task.Run → HandleMessageAsync(message)
    ...
    │   │   ├─ Everyone → 通过
    │   │   ├─ BotOwner → ownerQQ 或 机器人自身(SenderId == SelfId)
    │   │   ├─ Owner → 群主 或 ownerQQ 或 机器人自身
    │   │   └─ GroupAdmin → 管理员/群主 或 ownerQQ 或 机器人自身
...
#### 插件引用 ASP.NET Core 运行时（FrameworkReference）

插件可通过 `<FrameworkReference Include="Microsoft.AspNetCore.App" />` 引用 ASP.NET Core 运行时。
由于 MCT 宿主本身包含 ASP.NET Core 运行时，插件 ALC 在解析依赖时能从默认 ALC 中找到
`Microsoft.AspNetCore.*` 相关程序集，因此 FrameworkReference 在插件中可用。

**前提条件**：宿主程序必须也引用了 ASP.NET Core 运行时（通过 FrameworkReference 或 PublishTrimmed=false），
否则插件 ALC 无法解析到这些程序集，加载将失败。

**注意**：如果宿主未包含 ASP.NET Core 运行时，插件需要在 csproj 中改为
`<PackageReference>` 方式引入 ASP.NET Core 包，但这会增加插件体积且可能导致 ALC 类型不兼容。

---

## 附录G：插件签名验证

### G-1：FetchPublicKeyAsync() — 拉取签名公钥

```
FetchPublicKeyAsync()
│
├─ 创建 HttpClient（跳过SSL验证，超时15秒）
├─ GET https://服务器/api/preview/MorningCat%2FPluginSignature.txt
│   ├─ 请求成功 → 解析 JSON
│   │   ├─ 提取 "content" 字段作为新公钥
│   │   ├─ 新公钥非空?
│   │   │   ├─ 是 → 新公钥 != 本地缓存公钥?
│   │   │   │   ├─ 是 → 更新 _publicKey，写入配置文件，Log.Info "公钥已更新"
│   │   │   │   └─ 否 → Log.Debug "公钥未变化"
│   │   │   └─ 否 → 跳过
│   │   └─ 无 content 字段 → 跳过
│   └─ 请求失败 → Log.Warning
│       ├─ 本地有缓存公钥? → Log.Info "使用本地缓存公钥"
│       └─ 本地无缓存公钥? → 后续 VerifyDll 将跳过验证（返回 true）
└─
```

### G-2：OnBeforeModuleLoad 签名验证流程

签名验证通过 ModuleManager 的 `OnBeforeModuleLoad` 事件实现，在模块加载前拦截。

```
OnBeforeModuleLoad(context)
│
├─ context.AssemblyPath 非空?
│   └─ 否 → return ModuleLoadAction.Continue（无路径则跳过验证）
│
├─ _signatureVerifier.VerifyDllByAssemblyPath(assemblyPath)
│   ├─ assemblyPath 以 ".disabled" 结尾? → return true（跳过）
│   └─ VerifyDll(dllPath)
│       │
│       ├─ _testMode == true? → return true（测试模式跳过验证）
│       │
│       ├─ _publicKey 为空? → return true（无公钥则跳过验证）
│       │
│       ├─ 文件不存在? → return false
│       │
│       ├─ 读取文件全部字节
│       │
│       ├─ 文件 < 4 字节? → return false（无签名数据）
│       │
│       ├─ 读取末尾4字节作为签名长度 (sigLength)
│       │   ├─ sigLength <= 0 或 > 文件长度-4? → return false（签名数据无效）
│       │   └─ 有效 → 继续
│       │
│       ├─ 分离 DLL 内容和签名
│       │   ├─ dllBytes = fileBytes[0 .. dllContentLength]
│       │   │   其中 dllContentLength = fileBytes.Length - 4 - sigLength
│       │   └─ sigStringBytes = fileBytes[dllContentLength .. dllContentLength+sigLength]
│       │
│       ├─ 解码签名
│       │   ├─ sigBase64 = UTF8.GetString(sigStringBytes)
│       │   └─ signatureBytes = Convert.FromBase64String(sigBase64)
│       │
│       ├─ RSA 验证
│       │   ├─ ImportFromPem(_publicKey) 导入公钥
│       │   └─ rsa.VerifyData(dllBytes, signatureBytes, SHA256, PKCS1)
│       │       ├─ 验证通过 → return true
│       │       └─ 验证失败 → return false
│       │
│       └─ 异常 → return false
│
├─ 验证结果 == true? → return ModuleLoadAction.Continue
└─ 验证结果 == false
    ├─ 记录到 _signatureFailedModules
    ├─ Log.Warning "签名验证失败"
    └─ return ModuleLoadAction.Skip（跳过加载）
```

### G-3：签名数据格式

签名数据附加在 DLL 文件末尾，格式如下：

```
┌──────────────────────────────┬────────────────────┬──────────┐
│       DLL 原始内容            │   Base64 签名字符串  │ 签名长度  │
│   (dllContentLength 字节)     │   (sigLength 字节)   │ (4字节)  │
└──────────────────────────────┴────────────────────┴──────────┘
                                                         ↑
                                                    Int32 小端序
                                                    = sigLength
...
- **签名长度**：文件末尾 4 字节，`BitConverter.ToInt32` 读取
- **签名字符串**：UTF-8 编码的 Base64 字符串，长度 = sigLength
- **DLL 内容**：`fileBytes[0 .. Length-4-sigLength]`
- **签名算法**：RSA + SHA256 + PKCS1 填充

### G-4：WebUI 签名状态展示

WebUI 通过 `GetSignatureStatus()` 展示模块签名状态：

```
GetSignatureStatus(moduleName, assemblyPath)
│
├─ _signatureVerifier == null? → return null（未启用签名验证）
├─ _signatureVerifier.IsTestMode? → return null（测试模式不展示）
├─ _signatureFailedModules 包含 moduleName? → return "Failed"
├─ assemblyPath 非空且非 .disabled? → return "Verified"
└─ 其他 → return null
```
...
## 附录H：国际化组件（I18nManager）

### H-1：InitializeDefault() — 默认初始化

在 Program.Main 和 MorningCatBot 构造函数中各调用一次，确保日志系统初始化前就有翻译可用。

```
InitializeDefault()
│
├─ _currentLang = "en"
├─ 加载内嵌 zh.yml → _embeddedTranslations（用于完整性对比）
├─ 加载内嵌 en.yml → _translations（当前翻译）
├─ Instance = this（设置全局静态实例）
└─ 不创建 lang 目录、不解压文件、不扫描语言
```

**设计意图**：配置加载前无法确定目标语言，但日志系统需要翻译文本。
默认加载 en 可确保所有 `I18nManager.S()` 调用输出英文而非翻译键。

### H-2：Initialize(lang, baseDir) — 完整初始化

在 MorningCatBot.StartAsync() 中调用，根据配置或 --lang 参数确定目标语言。

```
Initialize(lang, baseDir)
│
├─ _currentLang = lang
├─ _langDirectory = baseDir + "/lang"
├─ Instance = this
│
├─ 创建 lang 目录（不存在则创建）
│
├─ ExtractEmbeddedLang("zh") — 解压内嵌中文语言包
│   ├─ 本地文件不存在 → 直接解压
│   └─ 本地文件已存在 → 检查完整性（缺失键则覆盖）
│
├─ ExtractEmbeddedLang("en") — 解压内嵌英文语言包（同上）
│
├─ 加载内嵌 zh.yml → _embeddedTranslations（完整性基准）
│
├─ ScanAvailableLanguages() — 扫描 lang/ 目录下所有 *.yml
│
├─ 目标语言文件存在?
│   ├─ 是 → 继续
│   └─ 否 → 目标语言是 zh 或 en?
│       ├─ 是 → 继续（内嵌语言包已解压，一定存在）
│       └─ 否 → InitError = "Language pack not found: {lang}.yml" → return false
│
├─ LoadLangFile(langFile) → _translations
│
├─ 目标语言是 zh 或 en? → CheckAndRepairCompleteness(lang)
│   └─ 遍历 _embeddedTranslations 的键
│       ├─ 本地文件缺失键 → 用内嵌内容覆盖本地文件，重新加载 _translations
│       └─ 键完整 → 不处理
│
└─ return true
```
...
```
I18nManager.S(key, args) → Instance.T(key, args)
│
├─ _translations.TryGetValue(key, out value)?
│   ├─ 是 → args.Length > 0?
│   │   ├─ 是 → string.Format(value, args)（格式化失败则返回原始 value）
│   │   └─ 否 → return value
│   └─ 否 → return key（回退到键名）
```
...
Program.Main
│
├─ I18nManager.Instance.InitializeDefault()  ← 加载 en，确保日志可用
│
├─ Log.Name("MorningCat")
├─ Log.Debug(I18nManager.S("log.starting"))  ← 输出英文 "MorningCat starting..."
│
├─ new MorningCatBot(...)
│   └─ _i18n.InitializeDefault()             ← Bot 实例也初始化默认值
│
├─ await bot.StartAsync()
│   └─ _i18n.Initialize(lang, baseDir)       ← 完整初始化，切换到目标语言
│       ├─ lang = "zh" → 后续日志输出中文
│       └─ lang = "en" → 后续日志输出英文
│
└─ ...后续所有 I18nManager.S() / _i18n.T() 使用目标语言
```